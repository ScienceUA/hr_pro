...
from parser_service.parsing.models import PageType
...
class BaseParser:
    ...
    def _classify_page(self) -> PageType:
        ...
        if "access denied" in text_lower or (
            "cloudflare" in text_lower and "ray id" in text_lower
        ):
            return PageType.BAN
        ...
        # Если есть H1, но нет контейнера резюме -> это НЕ резюме (например,
        # главная)
    ...
    def _get_text_safe(
        self, element: Union[Tag, BeautifulSoup], selector: str
    ) -> Optional[str]:
        if not element:
            return None
        try:
    ...
    def _get_attr_safe(
        self, element: Union[Tag, BeautifulSoup], selector: str, attr: str
    ) -> Optional[str]:
        if not element:
            return None
        try:
