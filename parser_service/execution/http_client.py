...
from parser_service.config.headers import get_headers
...
class HttpClientFactory:
    ...
    Управляет жизненным циклом клиентов, лимитами concurrency и ротацией прокси.  # noqa: E501
    """
    ...
    def __init__(self, settings: Any, proxy_manager: Any):
        ...
        # Глобальный Rate Limiting должен обеспечиваться инфраструктурой (Cloud
        # Tasks).
    ...
    def _validate_library_capability(self):
        ...
        if "proxy" not in sig.parameters:
            raise RuntimeError(
                "CRITICAL: Installed httpx version does not support 'proxy' argument. "  # noqa: E501
    ...
    async def client(self) -> AsyncGenerator[httpx.AsyncClient, None]:
        ...
        # Внимание: ожидающие корутины потребляют память, но тяжелый Client еще
        # не создан.
        async with self._semaphore:

            # Jitter внутри семафора гарантирует, что мы замедляем именно активную обработку,  # noqa: E501
            # но "съедаем" часть пропускной способности ради распределения
            # нагрузки.
            await self._jitter()
            ...
            try:
                ...
                logger.error(
                    f"HTTP Client Init Failed. Proxy: {safe_proxy}. Error class: {  # noqa: E501
