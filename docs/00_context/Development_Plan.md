...
6.5 Интеграция и Запуск (Local Agent Runner)
Цель: Объединить модули интерпретации (6.1), поиска (6.2), анализа (6.3) и отчетности (6.4) в единый исполняемый пайплайн.
Действия:
- Скрипт `run_agent.py` (Interactive CLI).
- Полный цикл: User Input -> Criteria -> Crawling -> Analysis -> Report.
- Поддержка режимов: Mock (бесплатно) / Real (LLM).
