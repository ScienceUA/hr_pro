...
- получает резюме из выдачи Work.ua, rabota.ua, возможно добавление и других сайтов-источников;
- выполняет смысловое сопоставление резюме с требованиями вакансии;
- возвращает рекрутеру:
  - ссылки на резюме на сайте-источнике,
  - оценку соответствия,
...
- система не предназначена для массовой выгрузки данных и построения постоянного хранилища резюме.
...
### 3.1 Агент (Интерфейс и "Мозг")
Роль:
- Принимает запрос в свободной форме и интерпретирует его.
- Управляет выбором источников: определяет, какие сайты включить или исключить на основе команд пользователя.
- Формирует универсальный CriteriaBundle (критерии поиска).

### 3.2 Оркестратор (`run_agent.py`)
Роль:
- Управляет последовательным жизненным циклом поиска по всем источникам.
- **Preview & Control:** Собирает данные о количестве найденных резюме со всех адаптеров и выводит сводную таблицу (Work: X, Rabota: Y, LinkedIn: Z) перед началом сбора. **Исключенные пользователем источники в таблицу не выводятся.**
- **Отказоустойчивость:** Если один из источников возвращает ошибку, оркестратор продолжает работу с остальными. 
  - В соответствующий блок отчета включается Traceback ошибки, **строго очищенный от конфиденциальных данных** (API-ключи, пароли, токены).
  - **Fail Fast:** Если ВСЕ выбранные источники вернули ошибку, система немедленно прекращает работу для вмешательства архитектора.
- **Блочная обработка:** Передает данные на анализ поочередно, сохраняя структуру источников.

### 3.3 Адаптеры источников (`app/sources/`)
Роль:
- Изолированные файлы (по одному на источник): отвечают за формирование поисковой ссылки и сбор данных (парсинг или API).
- Каждый файл содержит собственный маппинг фильтров (`*_filters_map.json`) для перевода общих критериев в специфичные параметры сайта, **а также свой меппинг критериев нахождения данных на страницах результата поиска.**
...
- исполнитель обеспечивает детерминированный сбор данных с сайтов-источников по команде агента;
